...
char *updateMessageById(char *newMessage,int i) {
    char *buff = mx_strnew(1024);
    sprintf(buff, "%s%s%s%d%s", "UPDATE Messages set edit = true, Message = '",newMessage,"' WHERE id = (SELECT MAX(id) - ",i," FROM Messages)");
    return buff;
}

char *deleteMessageById(int i) {
    char *buff = mx_strnew(1024);
    sprintf(buff, "%s%d%s", "DELETE FROM Messages WHERE id = (SELECT MAX(id) - ",i," FROM Messages)");
    return buff;
}
